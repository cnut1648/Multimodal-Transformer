...
from src.models.components.timesformer import VideoModel
...
class BiModule(ModuleMetricMixin, LightningModule):
    ...
    def forward_with_stat(self, batch: dict):
        """
        same as forward but compute stat for `exchange_levels`
        """
        statistics_storage1, statistics_storage2 = [], []
        def StatHook1(module, input, output):
            """
            pytorch hook
            called in the order from 0th layer to last
            use Linear to compute statistics
            """
            ith = len(statistics_storage1)
            # (B, seq_len, hidden)
            hidden = input[0]
            # (B, seq_len, hidden)
            representation = self.aligner1[f'{ith}th_aligner'](hidden)
            μ = representation.mean([0, 1])
            σ = representation.var([0, 1]).sqrt()
            statistics_storage1.append([μ, σ])
        
        def StatHook2(module, input, output):
            ith = len(statistics_storage2)
            # (B, #patch, hidden)
            hidden = input[0]
            # (B, seq_len, hidden)
            # representation = self.aligner2[f'{ith}th_aligner'](input)
            representation = self.aligner2[f'{ith}th_aligner'](hidden)
            μ = representation.mean([0, 1])
            σ = representation.var([0, 1]).sqrt()
            statistics_storage2.append([μ, σ])

        hook_handles = []
        for idx, layer_before_norms, _, is_exchange_enables in self.hparams.exchange_levels:
            for layername, hook, model, is_exchange_enable in zip(
                layer_before_norms, [StatHook1, StatHook2], [self.model1, self.model2],
                is_exchange_enables
            ):
                if is_exchange_enable:
                    ...
                    hook_handles.append(block.register_forward_hook(hook))
        logits = self(batch)
    ...
    def update_stat(self, statistics_storages):
        """
        statistics_storages: 2-tuple of [(mean, std)]
            [0]: gen from model1 and apply to model2
            [1]: gen from model2 and apply to model1
        """
        for idx, (_, _, normlayers, is_exchange_enables) in enumerate(self.hparams.exchange_levels):
            # normlayer: [model1 norm, model2 norm]
            # statistics_storages: [model1's output (in model2 space), model2's output (in model1 space)]
            for normlayer, mean_std_list, opposite_model, is_exchange_enable in zip(
                normlayers[::-1], statistics_storages,
                [self.model2, self.model1], is_exchange_enables):
                if is_exchange_enable:
                    mean, std = mean_std_list[idx]
                    norm = opposite_model.blocks[idx]
                    norm = get_module_by_name(norm, normlayer)

                    norm.weight.data = std
                    norm.bias.data = mean
    ...
    def compute_step(self, batch: dict, split: str):
        ...
        self.update_stat(statistics_storages)
        # only when link_mode == "next" and split == "train" will skip forward twice
        if self.hparams.link_mode != "next" or split != "train":
