...
class MultiDataset(Dataset):
    ...
    def get_text(self, utterance):
        ...
        text_dir = os.path.join(
            self.data_root, "text", f"Ses0{session}", f"{utterance}.txt"
        )
    ...
    def get_raw_audio(self, utterance, max_audio_len):
        ...
        audio_path = os.path.join(
            self.data_root, "raw_audio", f"Session{session}", "sentences", "wav",
...
class DataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        train, val, test = FOLD_META[self.hparams.fold]
        for folds, split in zip([train, val, test], ["train", "val", "test"]):
